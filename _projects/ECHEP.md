...
This notebook is _optional_, you can continue to notebook 4 without loosing any major information.
...
where `X` and `USER` depends on _YOUR_ user. For instance if your cern user is agomez then `X=a` and `USER=agomez`.
